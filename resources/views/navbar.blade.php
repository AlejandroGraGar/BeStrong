    ...
    <div class="max-w-7xl mx-auto flex items-center justify-between px-6 py-4">
        ...
        <a href="{{ url('/') }}" class="flex items-center gap-3 group">
            <div class="p-2 bg-white/20 backdrop-blur rounded-xl group-hover:bg-white/30 transition-colors">
                <img src="{{ asset('storage/logo.png') }}" alt="Logo" class="h-11 w-11">
            </div>
        ...
        <form method="POST" action="{{ route('logout') }}">
            @csrf
            <button type="submit" class="px-4 py-2 text-white/90 hover:text-white hover:bg-red-700/40 rounded-lg font-medium transition-all">
                Cerrar sesión
            </button>
        </form>
    </div>
